...
Если в РФ репозиторий недоступен (403/блокировки), используйте установку из .deb:
```bash
...
В скрипте `scripts/universal-install-update.sh` добавлена опциональная установка Prometheus и Grafana. По умолчанию включена.
...
# или
bash scripts/universal-install-update.sh --self-update
